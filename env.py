...
MODEL_ARCHITECTURE = {
        "SHARED_LAYER_SIZES": [int(x) for x in sys.argv[1:]], # Sizes of the hidden layers in the network that stddev and classification share
        "STDDEV_LAYER_SIZES": [], # Sized of the hidden layers for the stddev-specific head of the network (can be empty)
        "CLASS_LAYER_SIZES": [],  # Sized of the hidden layers for the classification head of the network (can be empty)
}
