...
class CustomLoss(nn.Module):
    ...
    def dist_type(self, vec, dim_count):
        ...
        curr_vec = vec[(dim_count-1)*(NUM_DISTS+2):dim_count*(NUM_DISTS+2)-2]
        # find the index of "1"
    ...
    def forward(self, pred, y):
        ...
        while dim_count <= NUM_DIMENSIONS:
            mean_loss += torch.abs(
                # dim_count*(env.NUM_DISTS+2) counts through the concatted dist vectors
                # then subtract 2 to get the mean
                dist_vec_actual[dim_count*(NUM_DISTS+2) - 2] 
                - 
                dist_vec_guess[dim_count*(NUM_DISTS+2) - 2]
            )
            stddev_loss += torch.abs(
                # same deal, just subtract 1 to get stddev instead
                dist_vec_actual[dim_count*(NUM_DISTS+2) - 1] 
                - 
                dist_vec_guess[dim_count*(NUM_DISTS+2) - 1]
            )
            # Search the lookup table, get the number of the dists in the list first
        ...
        return (mean_loss + stddev_loss) / (2 * NUM_DIMENSIONS * (1- (dist_loss/NUM_DIMENSIONS)))
